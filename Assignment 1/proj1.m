...
Grand = @(N) gamrnd(const1, const2, N, 1); 
gjoint = @(v1, v2) gampdf(v1, const1, const2) .* gampdf(v2, const1, const2) ;
...
%Calculating the expected value of the combined turbine
omega = f(draw)./g(draw);
...
drawV1 = Grand(N);
drawV2 = Grand(N);
omega1 = f(drawV1)./g(drawV1);
omega2 = f(drawV2)./g(drawV2);
sum = P(drawV1).*omega1 + P(drawV2).*omega2;
prob1 = length(find(sum > 3.075e6))/N;
prob2 = length(find(sum < 3.075e6))/N;
probtot = prob1 + prob2;